...
* Un noeud *Person*
* Un noeud *Product*
* Une relation *:ACHETE* entre Person et Product

image::../_images/reco_produit.png[Recommandation produits]

Il suffit d'une simple visualisation du graphe pour s'apercevoir que Judd et Geoffrey ont acheté deux produits en commun. Nous pourrions donc recommander à Judd les autres produits que Geoffrey a acheté.

Et en Cypher, il est tout aussi facile de le faire que de le dire :

[source,cypher]
----
MATCH (judd:Person {firstname:'Judd'})
MATCH (geoffrey:Person {firstname:'Geoffrey'})
MATCH (judd)-[:ACHETE]->(produit:Product)<-[:ACHETE]-(geoffrey)-[:ACHETE]->(produit2)
WHERE NOT (judd)-[:ACHETE]->(produit2)
RETURN judd, produit2 as recommandations
----
